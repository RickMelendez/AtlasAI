...
export function CircularLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("border-white/70 animate-spin rounded-full border-2 border-t-transparent", sizeClasses[size], className)}>
    ...
    <div className={cn("relative", sizeClasses[size], className)}>
      <div className="absolute h-full w-full">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className="bg-white/70 absolute rounded-full"
            style={{
              top: "0",
              left: "50%",
...
export function PulseLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("relative", sizeClasses[size], className)}>
      <div className="border-white/70 absolute inset-0 animate-[thin-pulse_1.5s_ease-in-out_infinite] rounded-full border-2" />
      <span className="sr-only">Loading</span>
    </div>
  )
}
...
export function PulseDotLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("bg-white/70 animate-[pulse-dot_1.2s_ease-in-out_infinite] rounded-full", sizeClasses[size], className)}>
      <span className="sr-only">Loading</span>
    </div>
  )
}
...
export function DotsLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("flex items-center space-x-1", containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div key={i} className={cn("bg-white/70 animate-[bounce-dots_1.4s_ease-in-out_infinite] rounded-full", dotSizes[size])}
...
export function TypingLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("flex items-center space-x-1", containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div key={i} className={cn("bg-white/70 animate-[typing_1s_infinite] rounded-full", dotSizes[size])}
...
export function WaveLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("flex items-center gap-0.5", containerSizes[size], className)}>
      {[...Array(5)].map((_, i) => (
        <div key={i} className={cn("bg-white/70 animate-[wave_1s_ease-in-out_infinite] rounded-full", barWidths[size])}
...
export function BarsLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("flex", containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div key={i} className={cn("bg-white/70 h-full animate-[wave-bars_1.2s_ease-in-out_infinite]", barWidths[size])}
...
export function TerminalLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("flex items-center space-x-1", containerSizes[size], className)}>
      <span className={cn("text-white/70 font-mono", textSizes[size])}>{">"}</span>
      <div className={cn("bg-white/70 animate-[blink_1s_step-end_infinite]", cursorSizes[size])} />
      <span className="sr-only">Loading</span>
    </div>
  )
}
...
export function TextBlinkLoader({ text = "Thinking", className, size = "md" }: { text?: string; className?: string; size?: "sm" | "md" | "lg" }) {
  const textSizes = { sm: "text-xs", md: "text-sm", lg: "text-base" }
  return (
    <div className={cn("animate-[text-blink_2s_ease-in-out_infinite] font-medium text-white/70", textSizes[size], className)}>
      {text}
    </div>
  )
}
...
export function TextShimmerLoader({ text = "Thinking", className, size = "md" }: { text?: string; className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(to right, rgba(255,255,255,0.4) 40%, rgba(255,255,255,0.9) 60%, rgba(255,255,255,0.4) 80%)",
...
export function TextDotsLoader({ className, text = "Thinking", size = "md" }: { className?: string; text?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("inline-flex items-center", className)}>
      <span className={cn("text-white/70 font-medium", textSizes[size])}>{text}</span>
      <span className="inline-flex">
        <span className="text-white/70 animate-[loading-dots_1.4s_infinite_0.2s]">.</span>
        <span className="text-white/70 animate-[loading-dots_1.4s_infinite_0.4s]">.</span>
        <span className="text-white/70 animate-[loading-dots_1.4s_infinite_0.6s]">.</span>
      </span>
    </div>
  )
}
